...
static int printfrequency = 50;
module_param(printfrequency, int, 0640);
MODULE_PARM_DESC(printfrequency, "the frequency used to print sch->qstats.backlog");
...
static int htb_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	sch->q.qlen++;
        ...
        //printk(KERN_INFO "qlen2:%u,sch->q.qlen:%u\n", sch->qstats.backlog, sch->q.qlen );
        printk(KERN_INFO "sch->qstats.backlog=%u\n", sch->qstats.backlog );
        }
        queuelength=sch->qstats.backlog;
        if(sch->qstats.backlog > threshold) {
            //printk(KERN_INFO "qlen>%d", threshold);
            if(ntohs(skb->protocol) == ETH_P_IP) {
                nh = ip_hdr(skb);
                //printk(KERN_INFO "nh = ip_hdr(skb)");
                if(nh->protocol == IPPROTO_TCP) {
                    ipv4_change_dsfield(nh, 0, 3);
                    //printk(KERN_INFO "queuelength=%u>%d and dsfield!\n", queuelength, threshold);
