...
>>>>>>> 0be3fa8e36b8d1d682542e3568ca3e104014a333:sch_htb.c
...
*/
/*
static int threshold = 10240;
module_param(threshold, int, 0640);
MODULE_PARM_DESC(threshold, "the threshold for htb_queue by bytes");
*/
static int queuemin = 10240;
module_param(queuemin, int, 0640);
MODULE_PARM_DESC(queuemin, "the min queue length");

static int queuemax = 20480;
module_param(queuemax, int, 0640);
MODULE_PARM_DESC(queuemax, "the max queue length");
...
=======
>>>>>>> 0be3fa8e36b8d1d682542e3568ca3e104014a333:sch_htb.c
static unsigned long avgqueue = 0;
...
static inline int ecn_prob(unsigned long qavg)
{
<<<<<<< HEAD:sch_htb_red.c
	return !((qavg-queuemin)*(qavg-queuemin)/(delta * delta) > (rand()%100)/double(100));
//	return !((qavg-queuemin)*(qavg-queuemin)/(delta * delta) > 0.5);
	}
=======
	int delta = queuemax - queuemin;
	struct reciprocal_value R = reciprocal_value(42949673);
	u32 a = reciprocal_divide (prandom_u32(),R);
	return !((qavg-queuemin)*(qavg-queuemin)*100/(delta * delta) > a);
}
>>>>>>> 0be3fa8e36b8d1d682542e3568ca3e104014a333:sch_htb.c

static inline unsigned long calc_qavg(unsigned long backlog)
{
	return avgqueue + ((backlog - avgqueue) >> 3);
}
...
// new ecn marking
	...
	//marking code;
	avgqueue = calc_qavg(sch->qstats.backlog);
//        if(sch->qstats.backlog > qavg) {
        if(sch->qstats.backlog > queuemin) {
        	printk(KERN_INFO "markprobability>%d", ecn_prob(avgqueue));
        	if(ntohs(skb->protocol) == ETH_P_IP) {
                	nh = ip_hdr(skb);
                	//printk(KERN_INFO "nh = ip_hdr(skb)");
                if(nh->protocol == IPPROTO_TCP && ecn_prob(avgqueue)) {
