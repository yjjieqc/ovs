...
static int k_1 = 30;
module_param(k_1, int, 0640);
MODULE_PARM_DESC(k_1, "the threshold for K1 by bytes");

static int k_2 = 36;
module_param(k_2, int, 0640);
MODULE_PARM_DESC(k_2, "the threshold for K2 by bytes");
...
static struct sk_buff *htb_dequeue(struct Qdisc *sch)
{
	...
	/*update average queue and new prevstart time*/
	long s;
	s64 delta;
	struct iphdr *nh;
...
ok:
	...
	for (level = 0; level < TC_HTB_MAXDEPTH; level++) {
		...
		delta = ktime_to_ns(ktime_sub(ktime_get(), prevstart));
		printk(KERN_INFO "delta t is %lld", delta);
//		delta = ktime_us_delta(ktime_get(), prevstart);
		if (delta < 1)
			printk(KERN_ERR "delta is zero, illegial divisor!");
		else
			s = (sch->q.qlen - prevqueue)/delta;
	}
	qavg = qavg + ((s - qavg) >> 3);
	if ((sch->q.qlen < k_2) && ((sch->q.qlen > k_1)|| (qavg < 0)))
            if(ntohs(skb->protocol) == ETH_P_IP) {
                nh = ip_hdr(skb);
